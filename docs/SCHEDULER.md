...
3. **Lease de Execução Própria (5 Minutos)**: O agendamento possui `executionToken` (UUID gerado para cada tentativa de execução) e `leaseExpiresAt` (5 minutos). Todas as transições de finalização exigem casamento exato de `executionToken`, `version` e `status: PROCESSING`.
   - **Justificativa Técnica da Lease de 5 Minutos**:
     - _Polling da Meta_: O polling de processamento assíncrono de containers de mídia no Instagram/Facebook dura no máximo 90 segundos (30 tentativas a cada 3 segundos). Uma lease de 5 minutos (300.000 ms) oferece margem de mais de 3x sobre o timeout máximo da Meta, evitando falsas expirações durante uploads e renderizações remotas.
     - _Lock do BullMQ_: O lock padrão do BullMQ é de 30 segundos, renovado a cada 15 segundos. A lease no PostgreSQL atua como camada de proteção persistente desacoplada do Redis.
     - _Recuperação Rápida (MTTR)_: Em caso de queda ou terminação abrupta do worker, o agendamento abandonado é descoberto e reconciliado em 5 minutos, sem deixar a publicação aguardando 10 minutos desnecessariamente.
...
| **SocialAccount**       |   ✅   |   ❌   | ❌ (via RLS direta) / ✅ (via função dedicada) |   ❌   | `social_account_read`: `can_read_client`<br>`social_account_update`: `can_edit_client` (revertida; APPROVER e scheduler bloqueados de editar metadata).<br>Expiração pelo scheduler feita estritamente via função `SECURITY DEFINER` `mark_social_account_expired(id)`. | Leitura da conta. Atualização direta restrita a administradores com `can_edit_client`. Em erro 190 da Meta, o scheduler invoca `mark_social_account_expired()` que altera exclusivamente `status = 'EXPIRED'` e `updatedAt = NOW()`. |
...
### Política de Retries, CAS de Reenfileiramento e Backoff

- **Concorrência**: 5 workers concorrentes por processo (configurável).
- **Tentativas máximas**: 4 tentativas para erros transitórios (`attemptsMade` persistido no agendamento).
- **Transição Segura CAS Pré-Retry**:
  - Quando um erro transitório ocorre e restam tentativas (`attemptsMade < maxAttempts - 1`), o worker executa um CAS exigindo o `executionToken` atual e `status: "PROCESSING"`, transicionando o agendamento de volta para `ENQUEUED`, limpando `executionToken = null` e `leaseExpiresAt = null`, e incrementando `attemptsMade`.
  - Somente após essa transição persistente no PostgreSQL o erro é propagado ao BullMQ, permitindo que a próxima execução do job encontre o agendamento em estado elegível (`ENQUEUED`) e adquira um novo token de execução sem colidir com locks antigos.
- **Preservação de Sucessos Parciais**:
  - Caso o post tenha sido agendado para múltiplas contas (ex: Facebook e Instagram) e uma delas obtenha sucesso (`PUBLISHED`) enquanto outra falha transitoriamente, a tentativa concluída é preservada no banco de dados.
  - No retry subsequente, o serviço de preparação identifica a conta já publicada (`allowSkippingPublished: true`), ignora-a e envia apenas a conta pendente para publicação remota.
- **Proteção Estrita contra Retries em Estados Incertos (`UNCERTAIN`)**:
  - Destinos em estado incerto (`UNCERTAIN`, como timeout de polling de mídia) **nunca são retentados automaticamente**.
  - O agendamento é imediatamente transicionado para `REQUIRES_RECONCILIATION`, a lease é liberada e o retry automático no BullMQ é cancelado para evitar duplicidade de postagens na Meta.
- **Esgotamento de Tentativas (`DEAD_LETTER`)**:
  - Ao atingir o limite de tentativas sem sucesso, o worker executa CAS para `DEAD_LETTER` (ou `PARTIALLY_PUBLISHED` se houve ao menos uma publicação concluída), gravando auditoria e finalizando o ciclo de vida.
